...
sim_wfe2 <- function (N = 100, Time = 20, lag.one = 4, lag.two = 6,
  ...
  for (i in 1:N) {
    ...
    eps[1, i] <- rnorm(1)
    y[1,i] <- rho_1*y.lagged[1,i] + alphai[i] + gammat[1] + rnorm(1)
  ...
  ols <- plm(y~treat + treat_l1 + y_l1 + x, 
